<?php
// test_specific_rfid_discount.php - Test discount retrieval for RFID I4OPS)V5
error_reporting(E_ALL);
ini_set('display_errors', 1);

require_once __DIR__ . '/../config/database.php';

echo "<h2>Testing Discount Retrieval for RFID: I4OPS)V5</h2>";

try {
    $db = getDB();
    
    // Test 1: Find the booking for RFID I4OPS)V5
    echo "<h3>Test 1: Finding Booking for RFID I4OPS)V5</h3>";
    ...
    $stmt->execute(['I4OPS)V5']);
    $booking = $stmt->fetch(PDO::FETCH_ASSOC);
    
    if (!$booking) {
        echo "<p style='color: red;'>❌ No booking found for RFID: I4OPS)V5</p>";
        exit;
    }
    ...
    echo "<h3>Test 2: Sales Transactions for I4OPS)V5</h3>";
    ...
    if (empty($transactions)) {
        ...
        echo "<p>This means the payment hasn't been processed yet, or there's an issue with the payment processing.</p>";
    } else {
    ...
    if ($emailReceiptData) {
        ...
        if ($discount > 0) {
            ...
            echo "<p style='color: orange;'>⚠️ No discount found - this might be why the email shows ₱1400</p>";
        }
    }
    
    // Test 4: Check if there are multiple transactions that might be causing confusion
    echo "<h3>Test 4: Transaction History Analysis</h3>";
    
    if (!empty($transactions)) {
        echo "<p>Transaction history analysis:</p>";
        
        $originalAmount = $booking['total_amount'];
        $hasDiscount = false;
        
        foreach ($transactions as $index => $txn) {
            $discountAmount = $txn['discount_amount'];
            $finalAmount = $txn['amount'];
            $expectedFinal = $originalAmount - $discountAmount;
            
            echo "<p><strong>Transaction {$txn['id']} ({$txn['created_at']}):</strong></p>";
            echo "<ul>";
            echo "<li>Original Amount: ₱{$originalAmount}</li>";
            echo "<li>Discount: ₱{$discountAmount}</li>";
            echo "<li>Final Amount: ₱{$finalAmount}</li>";
            echo "<li>Expected Final: ₱{$expectedFinal}</li>";
            echo "<li>Status: " . ($finalAmount == $expectedFinal ? "✅ Correct" : "❌ Incorrect") . "</li>";
            echo "</ul>";
        ...
        if ($hasDiscount) {
            echo "<p style='color: green;'>✅ Found transactions with discounts</p>";
        } else {
            echo "<p style='color: orange;'>⚠️ No transactions with discounts found</p>";
            echo "<p>This explains why the email receipt shows the original amount (₱1400) instead of a discounted amount.</p>";
        }
    }
